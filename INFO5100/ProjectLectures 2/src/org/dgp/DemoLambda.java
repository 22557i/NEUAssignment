...
public class DemoLambda {
	/*
	 * inner interface:
	 * Specification for a Lambda implementation.
	 * An Inner Interface CAN NOT be defined in an inner class.
	 */
	@FunctionalInterface 
	private interface FindAPerson {
		boolean found(DemoLambda.InnerPerson p);
	}
	/**
	 * inner class:
	 * 
	...
	public class InnerPerson {
		...
		public InnerPerson(int id, int age, String firstName, String lastName) {
			super();
			this.id = id;
			this.age = age;
...
//		showList(" integers in container in ascending order", numbers);	// call generic method with Integer types
	...
	void sortLambdaStrings() {
		...
		{
			List<String> names = new ArrayList<String>(Arrays.asList("Moe", "Larry", "Curley"));
			Collections.sort(names, (o1, o2) -> o1.compareTo(o2));		// lexagraphic ascending
//			Collections.sort(names, (String o1, String o2) -> o2.compareTo(o1));	// lexagraphic descending
...
//			.forEach(System.out::println);
	...
	public Boolean findAPerson(Predicate<InnerPerson> isPersonFound, InnerPerson p) {
		return isPersonFound.test(p);
	}
	
	public Boolean findAPerson(DemoLambda.FindAPerson isPersonFound, InnerPerson p) {
		return isPersonFound.found(p);
	}
	
	public void personLambda() {
		System.out.println("\n\n" + DemoLambda.class.getSimpleName() + ".personLambda()...");

		/*
		 * make a SimplePerson list
		 */
		InnerPerson[] a = { 
				new InnerPerson(),
				new InnerPerson(2,25,"Joe","Smith"), 
				new InnerPerson(3,72,"Albert","Peters"), 
				new InnerPerson(4,17,"Dan","Peters"),
				new InnerPerson(4,42,"John","Adams"), 
				new InnerPerson(5,34, "Dan","Dunn")     };
		List<InnerPerson> persons = new ArrayList<DemoLambda.InnerPerson>(Arrays.asList(a));
		
		System.out.println(persons.size() + " persons in list.");
		persons.forEach(System.out::println);
		
		FindAPerson foundDanPeters = (p) -> { return "Dan" == p.getFirstName() && "Peters" == p.getLastName(); };
		
		for (InnerPerson p : persons) {
			if (findAPerson(foundDanPeters, p)) System.out.println("\n\t FindAPerson FOUND: " + p);;
		}
		
		Predicate<InnerPerson> testDanPeters = (p) -> { return "Dan" == p.getFirstName() && "Peters" == p.getLastName(); };
		for (InnerPerson p : persons) {
			if (findAPerson(testDanPeters, p)) System.out.println("\n\t Predicate FOUND: " + p);;
		}
		
		System.out.println("\n\n" + DemoLambda.class.getSimpleName() + ".personLambda()... done!");
	}
	
	public int multiplyBy100(int n) {
		final int [] times100 = {0,100,200,300,400,500,600,700,800,900 };
...
//		
//		int n = 2;
//		System.out.println("Multiply " + n + " by 100 and result is: " + obj.multiplyBy100(n));
//		
//		n = 7;
//		System.out.println("Multiply " + n + " by 100 and result is: " + obj.multiplyBy100(n));
//		
...
//		n = 1;
//		System.out.println("Multiply " + n + " by 100 and result is: " + obj.multiplyBy100(n));
		System.out.println("\n\t" + DemoLambda.class.getSimpleName() + ".demo()... done!");
	}
}
